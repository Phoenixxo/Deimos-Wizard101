...
class ProductionImportTests(unittest.TestCase):
    ...
    def test_gui_close_uses_graceful_agent_cleanup(self):
        source = (REPOSITORY_ROOT / "Deimos.py").read_text(encoding="utf-8")
        shutdown_source = source[source.index("async def tool_finish"):]

        self.assertIn("agent_manager.stop", shutdown_source)
        self.assertIn("wizlaunch.clear_runtime()", shutdown_source)
    ...
    def test_macos_account_prompt_and_unauthenticated_launch_use_supported_threads(self):
        ...
        self.assertIn("account_prompt=(", source)
        self.assertIn(
    ...
    def test_questing_restart_and_dead_client_cleanup_preserve_lifecycle_state(self):
        ...
        self.assertIn("stalled_task, return_exceptions=True", source)
